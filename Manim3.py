...
def createChild(slf: Scene, parrentMojb: newDot, childMojb: newDot, isAnimation: bool):
    ...
    if isAnimation:
        ...
        if isinstance(vg, list):
            slf.play(AnimationGroup(*[MoveToTarget(n) for n in vg], lag_ratio=0))
        slf.play(FadeIn(pointer))
    else:
        moveChildren(slf, parrentMojb)
        slf.add(pointer)

def animateChildren(slf: Scene, parrentMojb: newDot, target: newDot): #need to be made into a transform method where it can collect all animation and then move.
    if len(parrentMojb.children)==0:
        return
    VGroup_list = list()
    parrentMojb.children.generate_target()
    parrentMojb.children.target.arrange_where_buffer_is_subtree_width(center= False).set_y(target.get_y()-1).align_to(target, RIGHT)
    VGroup_list.append(parrentMojb.children)
    for n in range(len(parrentMojb.children.submobjects)):
        ac = animateChildren(slf, parrentMojb.children.submobjects[n], parrentMojb.children.target.submobjects[n])
